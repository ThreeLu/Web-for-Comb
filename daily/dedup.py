...
def main():
    ...
    # Filter duplicates
    new_papers = [
        p for p in today_papers if p.get("id", "") not in history_ids
    ]
    dup_count = len(today_papers) - len(new_papers)
    print(
        f"Duplicates removed: {dup_count}, New: {len(new_papers)}",
        file=sys.stderr,
    )
    ...
    if new_papers:
        ...
        today_file.unlink()
        print(
            f"All papers are duplicates. Deleted {today_file}",
            file=sys.stderr,
        )
