...
function loadUserKeywords() {
  ...
  if (savedKeywords) {
    try {
      ...
      console.error('Failed to parse saved keywords:', error);
      userKeywords = [];
    }
  } else {
    // Use preset keywords as default
    userKeywords = typeof PRESET_KEYWORDS !== 'undefined' ? [...PRESET_KEYWORDS] : [];
  }
...
function loadUserAuthors() {
  ...
  if (savedAuthors) {
    try {
      ...
      console.error('Failed to parse saved authors:', error);
      userAuthors = [];
    }
  } else {
    // Use preset authors as default
    userAuthors = typeof PRESET_AUTHORS !== 'undefined' ? [...PRESET_AUTHORS] : [];
  }
...
async function fetchAvailableDates() {
  try {
    ...
    const dateRegex = /(\d{4}-\d{2}-\d{2})_ai_enhanced\.jsonl/;
...
async function loadPapersByDate(date) {
  ...
  try {
    ...
    const dataUrl = DATA_CONFIG.getDataUrl(`data/${date}_ai_enhanced.jsonl`);
...
function showPaperDetails(paper, paperIndex) {
  ...
  if (typeof renderMathInElement !== 'undefined') {
    try {
      ...
      console.warn('KaTeX modal rendering failed:', e);
    }
  }
