<div id=toc></div>

# Table of Contents

- [math.CO](#math.CO) [Total: 4]
- [math.PR](#math.PR) [Total: 1]


<div id='math.CO'></div>

# math.CO [[Back]](#toc)

### [1] [The Aldous property for normal Cayley graphs on symmetric groups](https://arxiv.org/abs/2607.29018)
*Chenhui Lv, Sanming Zhou*

Main category: math.CO

TL;DR: 本文研究了对称群 $S_n$ 上正规 Cayley 图的 Aldous 性质，证明了对于足够大的 $n$，几乎所有具有 Aldous 性质的正规 Cayley 图都被分类，并解决了 Li, Xia 和 Zhou 在 2023 年提出的一个开放问题。


<details>
  <summary>Details</summary>

Motivation: Aldous 谱隙猜想是随机游走与交换过程谱隙相等的著名猜想，其证明激发了对 $S_n$ 上其他 Cayley 图是否具有 Aldous 性质的广泛研究。本文旨在分类具有 Aldous 性质的正规 Cayley 图，填补了该领域对正规 Cayley 图 Aldous 性质完整分类的空白，并解决了 Li, Xia 和 Zhou 提出的开放问题。

Method: 本文使用了谱图论中的特征值分析、正规 Cayley 图的结构性质、以及关于第二特征值的精细估计。具体技术包括利用置换的支持大小来分类生成集，分析线图与补图的性质，并运用 Aldous 性质与特征值之间的等价条件。

Result: 主要结果包括：(1) 对于 $n \ge 5$，一个正规 Cayley 图及其补图至多一个具有 Aldous 性质，除非它们分别是 $2K_{n!/2}$ 和 $K_{n!/2,n!/2}$；(2) 对于足够大的 $n$，确定了所有具有 Aldous 性质的正规 Cayley 图 $\mathrm{Cay}(S_n, S)$，除了一个例外情况；(3) 证明了若 $S$ 中所有置换的支持大小为 $n-1$ 或 $n$，或包含所有支持大小为 $n$ 的置换，则非完全正规 Cayley 图不具有 Aldous 性质，从而解决了开放问题；(4) 确定了所有是线图的正规 Cayley 图，并分类了严格第二特征值不超过 1 的正规 Cayley 图。

Conclusion: 该结果对正规 Cayley 图的 Aldous 性质给出了近乎完整的分类，深化了对 $S_n$ 上 Cayley 图谱性质的理解，并为 Aldous 性质的研究提供了新的结构洞察。

Future Work: 论文中未解决的例外情况（即 $S$ 包含支持大小在 $\{2, \dots, n-2\}$ 和 $\{n-1, n\}$ 的置换，但不包含所有支持大小为 $n$ 的置换）是未来研究的主要方向。此外，可以探索将分类推广到更一般的群或非正规 Cayley 图。

Abstract: Aldous' spectral gap conjecture states that the random walk and the interchange process on any connected graph have the same spectral gap, or, equivalently, the second largest eigenvalue of any connected Cayley graph on the symmetric group $S_n$ with respect to a set of transpositions is achieved by the standard representation of $S_n$. This celebrated conjecture, proved in its general form in 2010, has inspired much interest in searching for other Cayley graphs on $S_n$ possessing this property, now known as the Aldous property. In this paper, we first prove that for $n \ge 5$ at most one of a normal Cayley graph on $S_n$ and its complement can possess the Aldous property except when these two graphs are $2K_{n!/2}$ and $K_{n!/2,n!/2}$ respectively. We then determine, for sufficiently large $n$, all normal Cayley graphs $\mathrm{Cay}(S_n, S)$ that have the Aldous property, except for the case when $S$ contains a permutation with support size in $\{2, 3, \dots, n-2\}$ and a permutation with support size in $\{n-1, n\}$, but not all permutations with support size $n$ are contained in $S$. In particular, we show that a non-complete normal Cayley graph $\mathrm{Cay}(S_n, S)$ does not have the Aldous property if all permutations in $S$ have support size $n-1$ or $n$, or all permutations with support size $n$ are contained in $S$, thereby solving an open problem posed by Li, Xia and Zhou in 2023. Along the way we determine all normal Cayley graphs on $S_n$ that are line graphs, and classify all normal Cayley graphs on $S_n$ with the strictly second largest eigenvalue at most $1$.

</details>


### [2] [Strong invariants and Tverberg numbers in convexity spaces](https://arxiv.org/abs/2607.29403)
*Minho Cho, Andreas F. Holmsen, Attila Jung, Hong Liu*

Main category: math.CO

TL;DR: 本文在一般凸性空间中建立了强Helly数、强Carathéodory数、强Radon数、comatching数与VC维数之间的等价性，并给出了Tverberg型分解、对偶性以及可分凸性空间中Tverberg数的线性上界。


<details>
  <summary>Details</summary>

Motivation: Helly、Carathéodory和Radon数是凸性空间中有限证书的基本度量，但它们的精确版本（强参数）尚未被系统研究。本文旨在统一这些强参数，并解决Calder-Eckhoff猜想和Tverberg数在可分凸性空间中的界的问题。

Method: 利用点与生成族之间的二分关联图，引入强参数与VC维数的等价性；通过唯一最小生成器构造对偶凸性空间；使用分层Tverberg分解和彩色化技术；对于可分凸性空间，结合Helly数和半空间VC维数给出Tverberg数的上界。

Result: 在任意凸性空间中，对于有限配置，五个有界性条件等价：VC维数、强Helly数、强Carathéodory数、comatching数和强Radon数（相差常数1）。此外，对于$S_3$-可分凸性空间，若Helly数为$h$且半空间VC维数为$d$，则$r_t=O(dh\log h)\,t$；特别地，$r_t=O(r^2\log r)\,t$。对于轴平行盒凸性，$r_t=O(rt)$，且维度无关。

Conclusion: 这些结果统一了凸性空间中的强参数，并首次给出了可分凸性空间中Tverberg数的线性上界，特别是盒凸性的维度无关估计，推进了Tverberg理论的发展。

Future Work: 论文提出了若干开放问题，例如改进可分凸性空间中Tverberg数的常数因子，以及探索强参数在其他凸性空间中的具体值。此外，将结果推广到更一般的凸性空间或高维配置也是自然的方向。

Abstract: Helly, Carathéodory, and Radon numbers encode three kinds of finite certificates in a convexity space: for the emptiness of an intersection, for membership in a convex hull, and for the existence of intersecting hulls. We study exact versions of these certificates, in which a subfamily must preserve the whole intersection or a subset must preserve the whole hull. Our first main result shows that, for finite configurations in an arbitrary convexity space, five a priori different boundedness conditions are equivalent: VC-dimension, strong Helly number, strong Carathéodory number, comatching number, and strong Radon number (with the expected additive-one shift). We also obtain equivalent layered Tverberg-type decompositions and colorful consequences. The common mechanism is exposed by the bipartite incidence graph between points and a generating family. For finite spaces, the unique minimal generator yields a natural dual convexity space; we characterize double dualization and prove that the strong parameters are duality invariant. The same model gives a polynomial-size, $O(t^4)$, realization of Bukh's counterexample to the Calder-Eckhoff partition conjecture. Finally, we obtain the first Tverberg bound for separable convexity spaces that is simultaneously linear in the number of parts and polynomial in the Radon number. If an $S_3$-separable convexity space has Helly number $h$ and its halfspaces have VC-dimension $d$, then $r_t=O(dh\log h)\,t$; in particular, Radon number $r$ gives $r_t=O(r^2\log r)\,t$. The bound attains the weak-Eckhoff scale $O(rt)$ whenever the Helly number is bounded. For axis-parallel box convexity in $\mathbb{R}^k$, gives the optimal order $r_t=O(rt)$ uniformly in every dimension. This appears to be the first dimension-uniform estimate of weak-Eckhoff order for box convexity, whereas the previous direct theory was confined to dimension three.

</details>


### [3] [Bipartite Extremal Numbers of Trees](https://arxiv.org/abs/2607.29579)
*Lucas Waite, Nuh Aydin*

Main category: math.CO

TL;DR: 本文研究二分宿主图上的树极值数（Erdős–Sós 问题的限制），给出自然下界构造并提出线性上界猜想，通过加权 $k$-minimality 方法证明了一类广泛树（包括扫帚树、两部分大小差至多 1 的树以及所有至多 7 个顶点的树）的上界，并解决了 Caro、Patkós 和 Tuza 提出的部分问题（至多差常数）。


<details>
  <summary>Details</summary>

Motivation: 经典的 Erdős–Sós 猜想断言任意 $n$ 顶点图若包含超过 $\frac{k-2}{2}n$ 条边则包含所有 $k$ 顶点树，但该猜想在一般图中仍未完全解决。本文将其限制在二分宿主图上，研究当宿主图仅给定阶数或两部分大小固定时的极值数，旨在理解二分结构对树嵌入的影响，并填补该方向的研究空白。

Method: 本文采用加权 $k$-minimality 的变体，结合自然下界构造（如完全二分图）和线性上界猜想，通过分析树的结构（如扫帚树、两部分大小差至多 1 的树）来证明上界。此外，还建立了固定部分极值数与普通极值数的关系，并引入了类似 Zarankiewicz 函数的有向二分极值函数。

Result: 主要结果包括：对于扫帚树、两部分大小差至多 1 的树以及所有至多 7 个顶点的树，在二分宿主图（仅给定阶数或两部分大小固定）中证明了线性上界（至多差常数），从而解决了 Caro、Patkós 和 Tuza 提出的部分问题。此外，还给出了自然下界构造，并提出了相应的线性上界猜想。

Conclusion: 该结果推进了二分图中树极值数的理解，表明加权 $k$-minimality 方法在二分设置中有效，并建立了固定部分极值与普通极值数的联系。然而，一般树的线性上界猜想仍未完全解决，需要进一步研究。

Future Work: 论文提出的线性上界猜想尚未完全证明，未来工作可尝试推广至更广泛的树族，或改进方法以消除常数差。此外，有向二分极值函数与 Zarankiewicz 函数的关系也值得进一步探索。

Abstract: We study a restriction of the classical Erdős--Sós problem, the extremal number of trees, to the class of bipartite host graphs, both when only the order of the host is prescribed and when its two part-sizes are fixed. We give natural lower-bound constructions and formulate corresponding linear upper-bound conjectures. We apply a weighted variant of $k$-minimality to prove upper bounds for a broad family of trees including brooms, trees with part-sizes differing by at most one, and all trees on at most 7 vertices, resolving part of a problem of Caro, Patkós and Tuza up to additive constants. We also relate the fixed-part extremal number of a tree to the ordinary extremal number, and consider an oriented bipartite extremal function analogous to the Zarankiewicz function.

</details>


<div id='math.PR'></div>

# math.PR [[Back]](#toc)

### [4] [Minimal inversion of a permuton sample](https://arxiv.org/abs/2607.28992)
*Benoît Corsini*

Main category: math.PR

TL;DR: 本文研究了由平面分布采样点排序得到的置换的逆序图的最小度（称为 minimal inversion），证明了在正则性条件下其重标度后的尾概率呈指数衰减，并构造了分布使得最小度以任意 $n^\beta$ 阶增长。


<details>
  <summary>Details</summary>

Motivation: 逆序图是置换的一种自然图表示，其统计性质近年来受到关注。已有工作（如 Bhattacharya 和 Mukherjee）研究了均匀随机置换的逆序图度数，但尚未推广到非均匀采样情形。本文旨在将最小度（minimal inversion）的极限行为推广到由任意平面分布诱导的置换，填补了这一空白。

Method: 本文主要使用概率方法，包括对置换的逆序数进行精细的计数和估计，利用大偏差原理和正则性条件（如密度函数的正则性）来推导尾概率的指数衰减。具体技术涉及对点集排序的几何概率分析。

Result: 设 $\sigma$ 为从分布 $\mu$ 中采样 $n$ 个点并按坐标排序得到的置换，其逆序图的最小度为 $\delta_n$。在 $\mu$ 满足正则性条件下，存在 $\alpha>0$ 和常数 $c>0$，使得 $\mathbb{P}(\delta_n / n^{\alpha/(\alpha+1)} > t) \sim \exp(-c t^{\alpha+1})$。此外，对任意 $\beta\in[0,1]$，存在分布 $\mu$ 使得 $\delta_n$ 以 $n^\beta$ 阶增长（$\beta=0,1$ 由恒等和反恒等置换实现）。

Conclusion: 该结果将逆序图最小度的极限行为从均匀置换推广到一般分布，揭示了最小度可能以任意介于 $n^0$ 和 $n^1$ 之间的幂律增长，丰富了随机逆序图的理论。

Future Work: 论文未明确列出开放问题，但自然延伸包括：研究最小度对应的顶点结构、逆序图的其他统计量（如连通性、最大度）在非均匀采样下的行为，以及放宽正则性条件。

Abstract: Given a permutation $σ$, its corresponding \textit{inversion graph} is obtained by adding an edge between $i<j$ if and only if $σ(i)>σ(j)$. The first results on random inversion graphs come from Acan and Pittel, who studied the connected threshold for a uniform permutation with fixed inversion number, and Bhattacharya and Mukherjee, who mostly focused on the degrees of the graph when the permutation is chosen uniformly at random. In this work, we call \textit{minimal inversion} the minimal degree of the inversion graph and extend a theorem from Bhattacharya and Mukherjee to the case where the permutation is not only uniform, but obtained as the ordering of points sampled according to some distribution on the plane. Under regularity assumptions on the distribution, and for the appropriate $α>0$, we show that the probability that the minimal inversion rescaled by $n^{α/(α+1)}$ is larger than $t$ behaves like $\exp(-ct^{α+1})$ for some constant $c>0$ depending on the distribution. We further show that every $α>0$ admits at least one corresponding distribution, thus proving that the minimal inversion can asymptotically scale as $n^β$ for any $β\in[0,1]$ (the cases $β=0$ and $β=1$ being obtained via the identity and anti-identity permutations, among others).

</details>
