...
### [1] [Rainbow Turán numbers for paths of length four](https://arxiv.org/abs/2608.25079)
*Sylwia Antoniuk, Andrzej Grzesik, Magdalena Prorok, Andrzej Ruciński*

Main category: math.CO

TL;DR: 本文研究了在避免彩虹 $P_5$ 的条件下，最大化 $k$ 个图的总边数或最小边数的彩虹 Turán 数问题，确定了总边数情形下的精确值，并给出了最小边数情形下的渐近结果和猜想。


<details>
  <summary>Details</summary>

Motivation: 彩虹 Turán 问题是经典 Turán 问题的推广，在组合数学中具有重要意义。本文关注路径 $P_5$ 的彩虹 Turán 数，填补了该领域对特定小路径研究的空白。已有工作主要针对其他图类，而 $P_5$ 的情形尚未完全解决。

Method: 本文使用了构造性方法、概率方法、以及图分解和极值图论中的经典技巧。具体地，对于总边数情形，通过构造达到上界的图族并利用边计数论证；对于最小边数情形，采用渐近分析和猜想驱动的构造。

Result: 对于总边数情形，确定了所有 $k\le n-1$ 时的最大值（也给出了 $P_4$ 的结果）。对于最小边数情形，得到了 $k\in\{5,6,9\}$ 时的渐近值，并提出了对所有其他 $k$ 的猜想；此外，在完全性附加假设下解决了 $k=4$ 的情形。

Conclusion: 该结果推进了彩虹 Turán 数理论，特别是对路径 $P_5$ 的完整刻画，为后续研究提供了重要参考。最小边数情形下的猜想为未来工作指明了方向。

Future Work: 论文提出了对所有其他 $k$ 的最小边数情形的猜想，并指出在一般情形下解决 $k=4$ 的问题仍是开放的。自然延伸包括研究更长的路径或其他图类的彩虹 Turán 数。
...
### [2] [Bounded diameter covering of 2-colored complete bipartite graphs](https://arxiv.org/abs/2608.25081)
*Louis DeBiasio, András Gyárfás, Gábor N. Sárközy*

Main category: math.CO

TL;DR: 本文证明了每个2-边染色的完全二部图的顶点可以被两个单色子图覆盖，且每个子图的直径至多为3，这是最优的。


<details>
  <summary>Details</summary>

Motivation: 该问题与Henderson--Ryser猜想的二部图有界直径类比相关。DeBiasio等人此前证明了存在直径为4的覆盖，本文旨在改进这一上界至最优值3，从而完善该问题的研究。

Method: 本文采用构造性证明方法，通过分析2-染色完全二部图中单色连通分支的结构，并利用二部图的特殊性质（如顶点划分和邻域关系）来构造两个直径至多为3的单色子图覆盖所有顶点。

Result: 主要定理：对于任意2-边染色的完全二部图，其顶点集可以被两个单色子图覆盖，使得每个子图的直径至多为3。此外，该直径上界是最优的，因为存在例子表明直径2不可能。

Conclusion: 该结果将覆盖直径从4改进到最优的3，完全解决了该二部图有界直径覆盖问题，并进一步支持了Henderson--Ryser猜想的二部图类比。

Future Work: 论文未明确提及未来工作，但自然延伸包括：考虑更多颜色（如k-染色）的完全二部图覆盖问题，或研究一般图（非二部）的类似有界直径覆盖问题。
...
### [3] [Clique decompositions and covers for large graphs](https://arxiv.org/abs/2608.25233)
*Wentao Zhang*

Main category: math.CO

TL;DR: 本文证明了 Erdős 关于边分解为团的最小总代价猜想和 Dau-Milenkovic-Puleo 关于 t-团覆盖的猜想在 n 足够大时成立，并刻画了等号情形。


<details>
  <summary>Details</summary>

Motivation: Erdős-Goodman-Pósa 在 1966 年证明了任意 n 顶点图 G 的边可以被至多 floor(n^2/4) 个团覆盖，且平衡完全二部图达到紧性。Erdős 进一步猜想存在总代价至多 floor(n^2/4) 的边分解为团，其中 i-团代价为 i-1。另一个由 Dau, Milenkovic 和 Puleo 提出的推广猜想涉及覆盖所有 t-团所需的最少团数。这些猜想是图分解与覆盖领域的核心问题，与 Turán 数、团覆盖数等经典概念紧密相关。此前 Balogh 等人仅证明了渐近和分数版本，本文首次给出精确结果。

Method: 对于 Erdős 猜想，结合线性规划对偶和生成森林多面体，利用 Mantel 定理的加权稳定性形式，并围绕几乎平衡二划分给出显式分解算法。对于 t-团覆盖猜想，结合图移除引理和广义 Turán 稳定性，并针对接近 Turán 图 T_{n,t} 的图给出精确覆盖构造。

Result: 证明了 Erdős 猜想：对足够大的 n，任意 n 顶点图 G 存在边分解为团，总代价至多 floor(n^2/4)，且等号成立当且仅当 G 是平衡完全二部图。同时证明了 Dau-Milenkovic-Puleo 猜想：对任意 t≥4 和足够大的 n，任意 n 顶点图 G 的 t-团可以被至多 prod_{j=0}^{t-1} floor((n+j)/t) 个团覆盖，并刻画了等号情形（即 G 为 Turán 图 T_{n,t}）。

Conclusion: 该结果完全解决了两个长期存在的猜想（在 n 足够大的条件下），为团分解与覆盖问题提供了精确的极值结果，并展示了线性规划与稳定性方法在组合极值问题中的强大威力。等号情形的刻画深化了对极值结构的理解。

Future Work: 论文未明确提及未来工作，但自然延伸包括：确定使结论成立的最小 n 值，推广到超图或带权图，以及研究更一般的团覆盖与分解的代价函数。
...
### [4] [Cop numbers for subclasses of partial cubes](https://arxiv.org/abs/2608.25416)
*Zhaoman Huang, Yan-Ting Xie, Shou-Jun Xu*

Main category: math.CO

TL;DR: 本文研究了部分立方体子类（中位数图、单纯形图）的 cop number，给出了中位数图基于 tree-dimension 的上界，以及单纯形图的一般上下界，并确定了若干特殊单纯形图的精确值。


<details>
  <summary>Details</summary>

Motivation: Cops and Robbers 博弈是图论中的经典问题，cop number 是衡量图复杂性的重要参数。尽管许多基本图类的 cop number 已被确定，但部分立方体及其子类的精确结果相对较少。本文旨在填补这一空白，改进已有上界，并为特殊图类提供精确值。

Method: 本文主要使用组合和结构分析方法，结合 tree-dimension 和 hypercube embedding 的概念，以及 clique number 和 chromatic number 的估计。具体技术包括利用中位数图的树结构性质，以及单纯形图的构造性质。

Result: 1. 对于任意有限中位数图 $M$，建立 $c(M) \le \lfloor \frac{td(M)}{2} \rfloor + 1$，其中 $td(M)$ 是 tree-dimension，改进了 Crawford 和 Iršič Chenoweth 的界。2. 对于单纯形图 $S(G)$，证明 $c(S(G)) \ge \lceil \frac{\omega(G)}{2} \rceil$ 且 $c(S(G)) \le \chi(G)$。3. 作为应用，确定了 bipartite wheels、Fibonacci cubes 和 Lucas cubes 的 cop number 的精确值。

Conclusion: 本文的结果为部分立方体的 cop number 提供了新的上界，特别是中位数图的界显著改进，并给出了单纯形图的一般界，为后续研究提供了工具。这些结果加深了对部分立方体类中博弈论性质的理解。

Future Work: 论文未明确提及开放问题，但自然延伸包括：改进中位数图的上界使其更紧，确定更多单纯形图的 cop number，以及探索其他部分立方体子类的 cop number。
...
### [5] [Burning Steiner triple systems](https://arxiv.org/abs/2608.25627)
*Andrea C. Burgess, Peter H. Danziger, Caleb W. Jones, Trent G. Marbach, David A. Pike*

Main category: math.CO

TL;DR: 本文研究了Steiner三元系统（STS）中的图燃烧和惰性燃烧过程，给出了任意STS燃烧数的对数界，证明了任意整数ρ≥3存在燃烧数为ρ的STS，并揭示了惰性燃烧与STS维数的等价性。


<details>
  <summary>Details</summary>

Motivation: 图燃烧是一种模拟网络影响力传播的离散过程，将其扩展到超图有多种方式，其中燃烧和惰性燃烧是两种已被研究的变体。组合设计（如Steiner三元系统）作为具有特殊结构的超图，其燃烧性质尚未被系统研究。本文旨在填补这一空白，探索STS的燃烧和惰性燃烧数，并建立与已有概念（如维数）的联系。

Method: 本文主要使用构造性方法，包括直接构造具有特定燃烧数的STS，以及利用STS的代数结构（如仿射和射影三元系统）进行分析。此外，还利用了燃烧过程的上下界估计技术，如通过覆盖设计或独立集来给出对数界。

Result: 主要结果包括：1) 任意具有$v$个顶点的STS的燃烧数$b(S)$满足$\Theta(\log v)$；2) 对任意整数$\rho \geq 3$，存在一个STS使得其燃烧数等于$\rho$；3) 惰性燃烧数$b_L(S)$等于STS的维数$\dim(S)$；4) 存在STS使得$b(S)-b_L(S)$可以任意大；5) 对仿射和射影三元系统，给出了燃烧数和惰性燃烧数的精确或渐近值。

Conclusion: 这些结果揭示了STS的燃烧性质与其组合结构之间的深刻联系，特别是惰性燃烧与维数的等价性为研究STS的维数提供了新的视角。同时，燃烧数与惰性燃烧数之间的巨大差异表明这两种过程在超图上的行为有本质不同。

Future Work: 论文提出了若干开放问题，例如确定任意STS的燃烧数的精确渐近常数，以及对于其他类型的组合设计（如Kirkman三元系统）的燃烧性质。此外，探索惰性燃烧与维数等价性的进一步应用也是一个自然方向。
...
### [6] [On the isolation numbers in graph products](https://arxiv.org/abs/2608.25752)
*Boštjan Brešar, Douglas F. Rall*

Main category: math.CO

TL;DR: 本文研究了图的各种乘积（直接、强、字典序和笛卡尔积）中的隔离数，特别是 $\cal F$-隔离数，给出了上下界并确定了若干精确值。


<details>
  <summary>Details</summary>

Motivation: 隔离数是图论中一个重要的参数，与支配集和打包问题密切相关。此前对笛卡尔积和字典序积的隔离数已有研究，但直接积和强积的隔离数尚未系统探讨。本文旨在填补这一空白，并推广到一般图族 $\cal F$。

Method: 本文使用了多种组合技巧，包括利用开打包（open packings）建立下界，通过构造 $\cal F$-横贯（$\cal F$-transversals）和引入 $\cal F$-隔离图（$\cal F$-isolation graphs）来获得上界，并结合支配数和全支配数等参数进行估计。

Result: 主要结果包括：对于直接积，给出了 $\{K_{n_1,\ldots,n_d}\}$-隔离数的上界（用因子的隔离数和全支配数表示）和下界（基于开打包），并确定了若干无穷族直接积的精确值，例如 $\iota(P_{4\ell}\times C_{2k+1})=\ell(k+1)$。对于强积，证明了涉及 $2$-打包数的下界 $\iota(G\,\boxtimes\, H,{\cal F})$ 以及 $\iota(G\,\boxtimes\, H)$ 的上界。对于字典序积，在多种一般情形下确定了 $\cal F$-隔离数的精确公式，用第一个因子的支配数和全支配数表示。对于笛卡尔积，将先前结果推广到任意图族 $\cal F$。

Conclusion: 这些结果系统地推进了图乘积中隔离数的研究，揭示了隔离数与支配参数之间的紧密联系，并为后续研究提供了新的工具（如 $\cal F$-隔离图）。

Future Work: 论文未明确列出开放问题，但自然延伸包括：改进直接积和强积的上下界，研究其他图乘积（如替换积）的隔离数，以及探索 $\cal F$-隔离数在更一般图族中的性质。
...
### [7] [The neighbourhood convexity](https://arxiv.org/abs/2608.25912)
*Daniela Bubboloni, José Cáceres*

Main category: math.CO

TL;DR: 本文研究图上的邻域凸性（$n$-convexity），一种基于公共闭邻域闭包算子的新有限凸性空间，刻画了其凸几何的结构，并揭示了奇偶性、准星顶点与邻域预序之间的联系。


<details>
  <summary>Details</summary>

Motivation: 图上的凸性理论在组合优化和算法设计中具有重要应用，但已有的路径类凸性（如测地凸性、单形凸性）具有规范行为。邻域凸性作为新定义的凸性，表现出非规范性和非遗传性，其结构性质尚不清楚，因此值得系统研究。

Method: 本文采用结构分析方法，引入准星（quasi-star）概念，证明约简性质，通过移除一对顶点（其中一个为准星）来系统约简 $n$-convex geometry；并利用邻域预序 $P(G)$ 与 $n$-convex 集的上集关系进行刻画。

Result: 主要结果包括：$n$-convex geometry 包含星顶点当且仅当其顶点数为奇数；每个奇数阶 $n$-convex geometry 可由偶数阶的通过附加星顶点唯一构造；约简性质允许通过移除准星对来约简；在无星 $n$-convex geometry 中，准星恰为 $P(G)$ 的极大元；最后分类了 quasi-threshold 和 threshold 图的 $n$-convex geometries。

Conclusion: 该研究揭示了邻域凸性与图的结构参数（如顶点数奇偶性）之间的深刻联系，为凸性理论提供了新的非规范实例，并建立了与邻域预序的桥梁，丰富了图凸性理论。

Future Work: 论文未明确列出开放问题，但自然延伸包括：研究 $n$-convexity 在其他图类（如弦图、完美图）中的性质，探索其算法应用，以及推广到有向图或超图上的邻域凸性。
