...
### [7] [Proof of the Density Threshold Conjecture for Pinwheel Scheduling](https://arxiv.org/abs/2606.27104)
*Akitoshi Kawamura*

Main category: cs.DM

TL;DR: 证明了 pinwheel scheduling 的 density threshold 猜想：密度不超过 $5/6$ 的所有实例都可调度，解决 Chan–Chin (1993) 猜想。


<details>
  <summary>Details</summary>

Motivation: pinwheel scheduling 要求每个周期 $a_i$ 的任务至少每 $a_i$ 天执行一次；密度 $\sum1/a_i\le1$ 是必要条件，$5/6$ 阈值是长期猜想。

Method: 将问题推广到 fractional（非整数）周期，从而归约到一个大但有限的实例集并用计算机搜索调度。

Result: 密度 $\le5/6$ 的实例皆可调度；副产品包括两种周期且密度 $\le1$ 必可调度的简单证明，以及 bamboo garden trimming 的 $4/3$ 近似快速算法。

Conclusion: density threshold 猜想被证明，fractional 周期归约是关键思想。

Future Work: 给出不依赖计算机搜索的证明；研究 $5/6$ 与 $1$ 之间实例的可调度性刻画。
